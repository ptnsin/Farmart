...
export default function EmployeeShipping() {
  ...
  // เมนู "จัดการ" (MoreVertical) รายแถว — ใช้เปลี่ยน order.statusStep เป็น "ถึงจุดหมาย"/"จัดส่งสำเร็จ"
  ...
  // ปิดเมนู "จัดการ" รายแถวเมื่อคลิกนอกกล่อง (เหมือนกับเมนู filter ด้านบน)
  useEffect(() => {
  ...
  /**
   * เปลี่ยนสถานะการจัดส่งของ order ที่ผูกกับ shipment นี้
   * - "arrived" (ถึงจุดหมาย, step 3) ไม่มี status key ใน STATUS_META เลยต้องใช้
   *   PATCH /api/orders/:id/advance (orderModel.advanceOrderStep) ซึ่งเลื่อนไปทีละ step
   * - "delivered" (จัดส่งสำเร็จ, step 4) ใช้ PATCH /api/orders/:id/status
   *   เพราะมี status key ตรงอยู่แล้ว และจะ sync shipment.status เป็น "delivered" ให้ด้วย
   *   (ดู syncShipmentForOrder ใน orderController.js) ต่างจาก advance ที่ไม่ sync shipment
   */
  async function handleUpdateOrderStep(shipment, action) {
    if (!shipment.order || statusUpdatingId) return;
    setOpenActionMenuId(null);
    setStatusUpdatingId(shipment.id);
    try {
      const data =
        action === "arrived"
          ? await api.patch(`/api/orders/${shipment.order}/advance`)
          : await api.patch(`/api/orders/${shipment.order}/status`, { status: "delivered" });
      if (data?.order) {
        setOrdersById((prev) => ({ ...prev, [data.order.id]: data.order }));
      }
    } catch (err) {
    ...
    <div className="flex h-screen overflow-hidden bg-slate-50">
      ...
      <main className="flex-1 overflow-y-auto px-6 py-6 md:px-10">
        <EmployeeTopBar
          ...
          searchPlaceholder="ค้นหารหัสการจัดส่ง หรือชื่อลูกค้า"
        />
                        ...
                        <td className="px-6 py-3.5">
                          <div className="flex items-center justify-end gap-3 text-slate-400">
                            ...
                            <div className="relative">
                              <button
                                type="button"
                                aria-label="เปลี่ยนสถานะการจัดส่ง"
                                disabled={statusUpdatingId === s.id}
                                onClick={() =>
                                  setOpenActionMenuId((id) => (id === s.id ? null : s.id))
                                }
                              ...
                              {openActionMenuId === s.id && (
                                <div
                                  ref={actionMenuRef}
                                  className="absolute bottom-full right-0 z-10 mb-1 w-40 rounded-lg border border-slate-100 bg-white py-1 text-left shadow-lg"
                                >
                                  <button
                                    type="button"
                                    disabled={s.statusInfo.step !== 2}
                                    onClick={() => handleUpdateOrderStep(s, "arrived")}
                                    className="block w-full px-3 py-1.5 text-left text-xs text-slate-600 hover:bg-slate-50 disabled:cursor-not-allowed disabled:text-slate-300 disabled:hover:bg-transparent"
                                  >
                                    {STEP_LABELS[3]}
                                  </button>
                                  <button
                                    type="button"
                                    disabled={s.statusInfo.step !== 3}
                                    onClick={() => handleUpdateOrderStep(s, "delivered")}
                                    className="block w-full px-3 py-1.5 text-left text-xs text-slate-600 hover:bg-slate-50 disabled:cursor-not-allowed disabled:text-slate-300 disabled:hover:bg-transparent"
                                  >
                                    {STEP_LABELS[4]}
                                  </button>
